...
\begin{equation}
\resizebox{0.93\textwidth}{!}{$
\norm{Q_{\bm{w}} - \wt{Q}}_{\nu}^2 \leq \frac{1}{(1-\gamma)\delta}\left(2\norm{\wt{B}(\bm{w}^{*})}_{\nu}^2 + \frac{c_1}{N^2} + \frac{c_2 +  2\lambda\softmin_j\left\lbrace\frac{1}{2\sigma^2_p}\norm{\bm{w}^* - \bm{w}_j}\right\rbrace}{N} + \frac{\upsilon(\bm{w}^*) + c_3\sqrt{\log\frac{2}{\delta}}}{\sqrt{N}}\right)$}
\end{equation}
...
\end{align*}
Notice that, from now on, one can simply apply the proof of Thm. \ref{th:main-gaussian} with $\sigma_{max}=\sigma_{min}=\sigma^2_p$ and $\frac{1}{2}\norm{\bm{w}^* - \bm{\mu}_p}_{\bm{\Sigma}_p^{-1}}$ replaced by $\softmin_j\left\lbrace\frac{1}{2\sigma^2_p}\norm{\bm{w}^* - \bm{w}_j}\right\rbrace$. Thus, by redefining the three constants to $c_1 = \frac{1}{2}\gamma^2\kappa^2\bm{\phi}_{max}^2 + \frac{\lambda d}{\sigma^2_p}$, $c_2 = \bm{w}_{max}^2\bm{\phi}_{max}^2(1 + \gamma)^2 + \lambda d (\log\sigma^2_p + \log N) + 2\lambda\log\abs{\mathcal{W}_s}$\todo{$c_2$ actually contains $N$. Should we make this more explicit?}, and $c_3 = 2\frac{R_{max}^2}{(1-\gamma)^2}$, we can write that, with probability at least $1-2\delta$:
\begin{align*}
\norm{Q_{\bm{w}} - \wt{Q}}_{\nu}^2 \leq \frac{1}{(1-\gamma)\delta}\left(2\norm{\wt{B}(\bm{w}^{*})}_{\nu}^2 + \frac{c_1}{N^2} + \frac{c_2 +  2\lambda\softmin_j\left\lbrace\frac{1}{2\sigma^2_p}\norm{\bm{w}^* - \bm{w}_j}\right\rbrace}{N} + \frac{\upsilon(\bm{w}^*) + c_3\sqrt{\log\frac{2}{\delta}}}{\sqrt{N}}\right)
\end{align*}
\end{proof}
