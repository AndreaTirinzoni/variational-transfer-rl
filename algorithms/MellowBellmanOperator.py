...
class LinearQMellowBellman(MellowBellmanOperator):
    ...
    def compute_gradient_diag_hessian(self, mdp_samples, weights=None):
        if weights is None:
            ...
            mm_gradient, mm_diag_hess = self._gradient_and_diag_hess_mellow_max(mdp_samples[:, s_prime:s_prime + self._Q.state_dim])
            q_gradient = self._Q.compute_gradient(mdp_samples[:, 0:r])
            b_grad = self._gamma * mm_gradient - q_gradient
            bellman_grad = 2 * np.average(br * b_grad)
            bellman_hess = 2 * np.average(self._gamma * br * mm_diag_hess + b_grad ** 2)
    ...
    def bellman_residuals(self, mdp_samples, weights=None):
        if weights is None:
            ...
            return mdp_samples[:, r] + self._gamma * mmQ - Q
    ...
    def _gradient_and_diag_hess_mellow_max(self, states, weights=None):
        if weights is None:
            ...
            grad = np.sum(qs * np.exp(q_gradient), axis=1)/qs_sum
            return grad, diag_hess
