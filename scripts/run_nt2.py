...
from features.agrbf import build_features_gw,build_features_gw2
...
def run(mdp, seed=None):
    ...
    # Build the features
    rbf_sa = build_features_gw(gw_size, n_basis, n_actions, state_dim, action_dim)
    iden = Identity(K / n_actions, state_dim, n_actions)
    rbf_s = build_features_gw2(gw_size, n_basis, state_dim)
    # Create BellmanOperator
    operator = MellowBellmanOperator(kappa, tau, xi, gamma, K, action_dim)
    # Create Q Function
    ...
    dts = utils.generate_episodes(mdp, pi_u, n_episodes=random_episodes) if random_episodes > 0 else None
    dtsf = None
    if dts is not None:
        t, s, a, r, s_prime, absorbing, sa = utils.split_data(dts, state_dim, action_dim)
        old = rbf_sa(sa)
        sf = np.repeat(rbf_s(s), n_actions, axis=1)
        sfp = np.repeat(rbf_s(s_prime), n_actions, axis=1)
        dtsf = np.concatenate((t[:,np.newaxis],sf,a,r[:,np.newaxis],sfp,absorbing[:,np.newaxis]), axis=1)
        t, s, a, r, s_prime, absorbing, sa = utils.split_data(dtsf, K, action_dim)
        new = iden(sa)
        assert np.allclose(old,new)

    buffer = Buffer(buffer_size, K)
    n_init_samples = buffer.add_all(dtsf) if dtsf is not None else 0
    ...
    for i in range(max_iter):
        ...
        sf = np.squeeze(np.repeat(rbf_s(s), n_actions, axis=1))
        a = pi.sample_action(sf)
        # Step
        s_prime, r, done, _ = mdp.step(a)
        # Build the new sample and add it to the dataset
        sfp = np.squeeze(np.repeat(rbf_s(s_prime), n_actions, axis=1))
        buffer.add_sample(h, sf, a, r, sfp, done)
        ...
        if i % eval_freq == 0:
            ...
            #br = operator.bellman_residual(Q, dataset_feat) ** 2
            ...
            if verbose:
                print("Iter {} Episodes {} Rew(G) {} Rew(L) {} L2 {} L_inf {} Sft {} time {:.1f} s".format(
                    i, episodes[-1], 0, learning_rew, 0, 0, 0, elapsed_time))
